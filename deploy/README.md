...
这是 private 仓库。请先在浏览器登录拥有仓库读取权限的 GitHub 账户，再从固定的 [v0.2.0-dev.3 Release](https://github.com/arctan303/OneAPI/releases/tag/v0.2.0-dev.3) 页面下载 tar.gz 和对应 `.sha256` 资产。也可以使用已认证的 GitHub CLI：
...
Windows PowerShell 可用 `Get-FileHash .\oneapi-server-0.2.0-dev.3.tar.gz -Algorithm SHA256`，再与 `.sha256` 文件中的值比较。确认一致后解压并进入包内 `server/` 目录；包内已经包含运行 bundle，不需要 `npm install`：
...
正式 HTTPS、Caddy、systemd、升级和旧账号迁移见固定版本 [安装教程](https://github.com/arctan303/OneAPI/blob/v0.2.0-dev.3/docs/INSTALL.md) 与 [部署说明](https://github.com/arctan303/OneAPI/blob/v0.2.0-dev.3/docs/DEPLOYMENT.md)。若官方未返回 5 小时额度窗口，应显示未知；遇到目录/额度错误时保留状态码和应用错误码，不循环发送请求。

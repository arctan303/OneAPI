...
5. 模型测试填模型 ID 和消息后发送；外部软件则在 API 密钥区域创建命名密钥并复制当次显示的完整 key。
...
- [产品契约](Product-Spec.md)、[开发计划](DEV-PLAN.md)、[LIVE-001 验证](verification/LIVE-001.md)、[403 诊断历史](verification/403-runtime-diagnosis.md)。旧 DEMO/AUTH 证据维持原适用基线，不把 Mock 改写成真实成功。

## 尚未证明

Cloudflare 云端出口和部署、自然 token 刷新、全部目录模型逐一生成，以及 Chat/函数工具的真实上游行为仍待单独验收。真实目录可选不等于每个模型均已测试。

...
import { fetchModels, fetchResponseStream, fetchUsage, generationAbortError } from "./codex/upstream";
...
export class AccountDurableObject extends DurableObject<Env> {
  ...
  constructor(ctx: DurableObjectState, env: Env) {
    ...
    ctx.blockConcurrencyWhile(async () => {
      ctx.storage.sql.exec(`CREATE TABLE IF NOT EXISTS request_logs (
        id TEXT PRIMARY KEY,
      ...
      ctx.storage.sql.exec("CREATE INDEX IF NOT EXISTS request_logs_started_idx ON request_logs(started_at DESC)");
      ctx.storage.sql.exec("CREATE INDEX IF NOT EXISTS request_logs_key_idx ON request_logs(key_id, started_at DESC)");
  ...
  private async legacyPolicy(): Promise<ApiKeyPolicy> {
    return normalizePolicy(await this.ctx.storage.get<Partial<ApiKeyPolicy>>(LEGACY_POLICY_KEY));
  }
  ...
  private async patchApiKey(request: Request, id: string): Promise<Response> {
    ...
    if (id === LEGACY_KEY_ID) {
      ...
      await this.ctx.storage.put(LEGACY_POLICY_KEY, next);
  ...
  private async usage(force: boolean): Promise<Response> {
    const connected = await this.readCredentials();
    const fetchedAt = Date.now();
    if (!connected) {
      await this.ctx.storage.delete(USAGE_CACHE_KEY);
      return Response.json({
        available: false,
        fetchedAt,
        lastSuccessAt: null,
    ...
    const cached = await this.ctx.storage.get<UsageCacheRecord>(USAGE_CACHE_KEY);
    ...
    try {
      ...
      await this.ctx.storage.put(USAGE_CACHE_KEY, { accountId: credentials.accountId, snapshot } satisfies UsageCacheRecord);
  ...
  private async logSettings(): Promise<LogSettings> {
    const stored = await this.ctx.storage.get<Partial<LogSettings>>(LOG_SETTINGS_KEY);
    return {
  ...
  private async patchLogSettings(request: Request): Promise<Response> {
    ...
    await this.ctx.storage.put(LOG_SETTINGS_KEY, next);
    this.pruneLogs(next);
  ...
  private pruneLogs(settings: LogSettings): void {
    ...
    this.ctx.storage.sql.exec("DELETE FROM request_logs WHERE completed_at IS NOT NULL AND started_at < ?", summaryCutoff);
    this.ctx.storage.sql.exec(
  ...
  private async scheduleLogAlarm(settings?: LogSettings): Promise<void> {
    ...
    addDeadline(this.ctx.storage.sql.exec<{ deadline: number | null }>(
    ...
    const summaryStart = this.ctx.storage.sql.exec<{ startedAt: number | null }>(
    ...
    const activeStart = this.ctx.storage.sql.exec<{ startedAt: number | null }>(
  ...
  private async listLogs(url: URL): Promise<Response> {
    ...
    const rows = this.ctx.storage.sql.exec<Record<string, string | number | null>>(query, ...params, limit + 1, offset).toArray();
  ...
  private async loadModelCatalog(): Promise<ModelCapability[]> {
    ...
    const cached = await this.ctx.storage.get<unknown>(MODEL_CACHE_KEY);
  ...
  private async releaseLease(id: string): Promise<void> {
    const leases = (await this.ctx.storage.get<Record<string, number | LeaseRecord>>(LEASES_KEY)) ?? {};
    if (id in leases) {
      delete leases[id];
  ...
  async fetch(request: Request): Promise<Response> {
    ...
    try {
      ...
      if (request.method === "GET" && url.pathname === "/admin/usage") {
        ...
        return await this.usage(url.searchParams.get("refresh") === "true");
      }
